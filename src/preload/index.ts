...
const api = {
  ...
  // Credentials
  getCredentials: (projectId: string) => ipcRenderer.invoke('credential:get', projectId),
  ...
  // Apple IAP
  fetchAppleProducts: (projectId: string) => ipcRenderer.invoke('apple:fetch-products', projectId),
  ...
  updateAppleProduct: (projectId: string, iapId: string, updates: { referenceName?: string }) =>
  ...
  batchUpdateGoogleStatus: (
    projectId: string,
    ids: string[],
    active: boolean,
    products: unknown[]
  ) => ipcRenderer.invoke('google:batch-status', projectId, ids, active, products),
  getGoogleSettings: (projectId: string) => ipcRenderer.invoke('google:get-settings', projectId),
  ...
  getGoogleRegions: (projectId: string) => ipcRenderer.invoke('google:get-regions', projectId),
  ...
  setGoogleLegacyCompatible: (projectId: string, productId: string, purchaseOptionId: string) =>
    ipcRenderer.invoke('google:set-legacy-compatible', projectId, productId, purchaseOptionId),
  ...
  onExportProgress: (
    callback: (data: { current: number; total: number; phase: string }) => void
  ) => {
    const listener = (_event: any, data: any) => callback(data)
